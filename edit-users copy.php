...
if(isset($_REQUEST['submit']) and $_REQUEST['submit']!=""){
	...
	$update	=	$db->update('user',$data,array('id'=>$editId));
	if($update){
		header('location: browse-users copy.php?msg=rus');
		exit;
	}else{
		header('location: browse-users copy.php?msg=rnu');
		exit;
	}
}
?>
...
<body>
	...
	<div class="bg-light border-bottom shadow-sm sticky-top">
   	...
   	<div class="container">
		<h1><a href="https://learncodeweb.com/php/php-crud-in-bootstrap-4-with-search-functionality/">Manchester United</a></h1>
		<?php
		if(isset($_REQUEST['msg']) and $_REQUEST['msg']=="un"){
			...
			echo	'<div class="alert alert-danger"><i class="fa fa-exclamation-triangle"></i> Position play is mandatory field!</div>';
		...
		?>
		<div class="card">
			<div class="card-header"><i class="fa fa-fw fa-plus-circle"></i> <strong>Add Player</strong> <a href="browse-users.php" class="float-right btn btn-dark btn-sm"><i class="fa fa-fw fa-globe"></i> Browse Player</a></div>
			<div class="card-body">
				...
				<div class="col-sm-6">
					...
					<form method="post">
						<div class="form-group">
							...
							<input type="text" name="playername" id="playername" class="form-control" value="<?php echo $row[0]['playername']; ?>" placeholder="Enter player name" required>
						</div>
						<div class="form-group">
							<label>Position play <span class="text-danger">*</span></label>
							<input type="text" name="positionplay" id="positionplay" class="form-control" value="<?php echo $row[0]['positionplay']; ?>" placeholder="Enter Player position" required>
						</div>
						<div class="form-group">
							<label>Jersery Number <span class="text-danger">*</span></label>
							<input type="text" name="jerserynumber" id="jerserynumber" maxlength="12" class="form-control" value="<?php echo $row[0]['jerserynumber']; ?>" placeholder="Enter Jersery number" required>
						</div>
						<div class="form-group">
